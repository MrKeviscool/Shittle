...
template <typename Iter, typename PegGetter>
using PegIterator_t = typename std::decay<decltype(PegGetter::operator()(std::declval<typename std::iterator_traits<Iter>::value_type&>()))>::type::iterator;
...
namespace SaveLoadDetail {
	template <typename Iter, typename PegGetter>
	dword_t getThumbSize(const SerializedLevelWrite<PegIterator_t<Iter, PegGetter>>& lev) {
	...
	template <typename Iter, typename PegGetter>
	dword_t getLevelSize(const SerializedLevelWrite<PegIterator_t<Iter, PegGetter>>& lev) {
	...
	static dword_t writeThumbnailOffsets(std::fstream& file, const Iter startIter, const Iter endIter, dword_t curOffset) {
		...
		for (Iter lev = startIter; lev != endIter; ++lev) {
			fileStream::write<dword_t>(file, curOffset);
			SerializedLevelWrite <PegIterator_t<Iter, PegGetter>> serializedLev = *lev;
			curOffset += getThumbSize<Iter, PegGetter>(serializedLev);
		}
	...
	static void writeLevels(std::fstream& file, Iter startIter, Iter endIter) {
		...
		for (auto lev = startIter; lev != endIter; ++lev) {
			SerializedLevelWrite<PegIterator_t<Iter, PegGetter>> serialLev = *lev;
