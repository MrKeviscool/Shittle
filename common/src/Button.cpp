...
sf::Vector2f Button::calculateTextPosition() const {
	return {
		m_shape.getPosition().x + (m_shape.getSize().x/2) - (m_text.getGlobalBounds().width * 0.5f),
		m_shape.getPosition().y + m_shape.getSize().y * (!m_textIsInsideButton? 1 : 0.5f) + m_textOffsetAmount
	};
}
...
void Button::setText(const std::string& text, const sf::Font* font, const unsigned int size, const float offsetAmount){
	...
	m_textOffsetAmount = offsetAmount;
	const sf::Vector2f textPos = calculateTextPosition();
...
void Button::setTextInsideButton(const bool textInsideButton) {
	m_textIsInsideButton = textInsideButton;
}
