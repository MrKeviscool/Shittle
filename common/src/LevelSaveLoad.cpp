...
inline static dword_t getThumbSize(const SerializedLevel& lev) {
...
static dword_t getLevelSize(const SerializedLevel& lev) {
...
static dword_t getTotalThumbnailsSize(const std::vector<SerializedLevel>& levels) {
	dword_t size = 0;
...
static dword_t writeAmountOfLevels(std::fstream& file, const std::vector<SerializedLevel>& levels, const dword_t curOffset = 0) {
	fileStream::write<word_t>(file, static_cast<word_t>(levels.size()));
	return curOffset + sizeof(word_t);
}

static dword_t writeThumbnailOffsets(std::fstream& file, const std::vector<SerializedLevel>& levels, dword_t curOffset) {
...
static dword_t writeLevelOffsets(std::fstream& file, const std::vector<SerializedLevel>& levels, dword_t curOffset) {
	curOffset += static_cast<dword_t>(levels.size() * sizeof(dword_t) + 1);
...
static void writeThumbnails(std::fstream& file, const std::vector<SerializedLevel>& levels) {
	for (const auto& lev : levels) {
		...
		const sf::Uint8* imgBytes = lev.background.getPixelsPtr();
